...
var pageURL = 'admin/log_deleted_content.php'
...
function loadPage()	//the function that loads pages via AJAX
{
	...
	$('#deletedPageContent').html("<div class='center'>Loading...</div>");
	for (i=1; i <= pageMax; i++){
    ...
    $.ajax({	//create an ajax request to load_page.php
        ...
        success: function(msg){
            ...
            {
                $('#deletedPageContent').html(msg);	//load the returned html into pageContet
                //$('#loading').css('visibility','hidden');	//and hide the rotating gif
            }else{
				$('#deletedPageContent').html("error");
			}
        }
