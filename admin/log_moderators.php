<?php
	...
	if($FDBPrivilages['VMLog']==2){	
		$stmt = $Gamedb->prepare("SELECT * FROM `modlog` WHERE 1 ORDER BY `Date` DESC");		
		$stmt->execute();
...
var pageURL = 'admin/log_moderators_content.php'
...
function loadPage()	//the function that loads pages via AJAX
{
	...
	$('#modPageContent').html("<div class='center'>Loading...</div>");
	for (i=1; i <= pageMax; i++){
    ...
    $.ajax({	//create an ajax request to load_page.php
        ...
        success: function(msg){
            ...
            {
                $('#modPageContent').html(msg);	//load the returned html into pageContet
                //$('#loading').css('visibility','hidden');	//and hide the rotating gif
            }else{
				$('#modPageContent').html("error");
			}
        }
